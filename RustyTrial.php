    ...
    <div id="particles-js"></div>
        ...
        <div class="container d-flex w-100 h-100 p-3 mx-auto flex-column">
            ...
            <?php foreach($resultado as $row) { ?>
                <div class="col">
                  <div class="card shadow-sm" style="background-color:darkgray;">
                      <?php

                      $id = $row[('id')];
                      $imagen = "Images/CHALLENGE Y RUSTY TRIAL/CHALLENGE XTREME/".  $id . "/Principal.jpg" ;

                      if(!file_exists($imagen)) {
                      $imagen = "Images/KARMA3D.jpg";
                      }
                      ?>
                      <img src="<?php echo $imagen; ?>" class="d-block w-90" style="height: 250px;">
                      <div class="card-body" style="opacity: 1">
                      <h5 class="card-tittle"><?php echo $row['nombre']; ?></h5>
                      <p class="card-text">$ <?php echo number_format($row['precio'], 2, '.', ','); ?></p>
                      <div class="d-flex justify-content-between align-items-center">
                          <div class="btn-group">
                              <a href="detalles.php?id=<?php echo $row[('id')];?>&token=<?php echo hash_hmac('sha512', $row['id'], KEY_TOKEN);?>" class="btn btn-outline-success">Cotizar Ahora</a>
                          </div>
                      </div>
                      </div>
                  </div>
